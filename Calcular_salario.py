...
def gui_main():
    ...
    # =============================================================================
    # Validação simples para campos monetários
    # =============================================================================
    ...
    labels_basic = ["Salário Bruto (R$):", "Adiantamento Salário (%):", "Ano (ex: 2025):", "Mês (1-12):",
                    "Total Dias no Mês:"]
    entries_basic = []
    ...
    entries_basic[0].config(textvariable=salario_bruto_var, validate="all",
                            validatecommand=(vcmd, '%d', '%i', '%S', '%v', '%V', '%W'))
    entries_basic[1].config(textvariable=adiantamento_var)
    ...
    # Formatação do campo monetário ao perder foco
    def formatar_campo(event, sv):
        value = sv.get()
        if value:
            clean_value = value.replace('.', '').replace(',', '.')
            try:
                float_value = float(clean_value)
                sv.set(formatar_valor(float_value))
            except ValueError:
                # Se houver erro de conversão, mantém valor original
                pass

    entries_basic[0].bind("<FocusOut>", lambda event: formatar_campo(event, salario_bruto_var))

    # =============================================================================
    # Frame - Benefícios e Insalubridade (VR e dias)
    # =============================================================================
    benefits_frame = tk.LabelFrame(input_frame, text="Benefícios e Insalubridade", bg="#909090", fg="black",
                                   font=("Roboto", 12))
    benefits_frame.pack(fill="x", padx=10, pady=5)

    labels_benefits = ["Insalubridade (%):", "Valor VR por Dia (R$):", "Dias Úteis VR:"]
    entries_benefits = []
    ...
    entries_benefits[0].config(textvariable=insal_var)
    entries_benefits[1].config(textvariable=valor_vr_var, validate="all",
                               validatecommand=(vcmd, '%d', '%i', '%S', '%v', '%V', '%W'))
    entries_benefits[2].config(textvariable=dias_vr_var)
    entries_benefits[1].bind("<FocusOut>", lambda event: formatar_campo(event, valor_vr_var))
    ...
    row_disc += 1
    lbl_vr_perc = tk.Label(discounts_frame, text="% Desconto VR:", bg="#909090", fg="black")
    ent_vr_perc = tk.Entry(discounts_frame, textvariable=desconto_vr_perc_var, width=40, bg="#f0f0f0", fg="black")
    ...
    row_disc += 1
    lbl_vt_perc = tk.Label(discounts_frame, text="% Desconto VT:", bg="#909090", fg="black")
    ent_vt_perc = tk.Entry(discounts_frame, textvariable=desconto_vt_perc_var, width=40, bg="#f0f0f0", fg="black")
    ...
    row_disc += 1
    lbl_saude_perc = tk.Label(discounts_frame, text="% Desconto Saúde:", bg="#909090", fg="black")
    ent_saude_perc = tk.Entry(discounts_frame, textvariable=desconto_saude_perc_var, width=40, bg="#f0f0f0", fg="black")
    ...
    row_disc += 1
    lbl_odonto_perc = tk.Label(discounts_frame, text="% Desconto Odonto:", bg="#909090", fg="black")
    ent_odonto_perc = tk.Entry(discounts_frame, textvariable=desconto_odonto_perc_var, width=40, bg="#f0f0f0",
                               fg="black")
    ...
    def calcular():
        ...
        try:
            ...
            salario_bruto = limpar_formato(salario_bruto_var.get()) if salario_bruto_var.get() else 0.0
            adiantamento_salario = float(adiantamento_var.get() or 0)
            ano = int(ano_var.get() or 0)
            mes = int(mes_var.get() or 0)
            dias_mes = int(dias_mes_var.get() or 0)
            insal_perc = float(insal_var.get() or 0)
            valor_vr = limpar_formato(valor_vr_var.get()) if valor_vr_var.get() else 0.0
            dias_vr = int(dias_vr_var.get() or 0)
            desconto_vr_perc = float(desconto_vr_perc_var.get() or 0)
            desconto_vt_perc = float(desconto_vt_perc_var.get() or 0)
            desconto_saude_perc = float(desconto_saude_perc_var.get() or 0)
            desconto_odonto_perc = float(desconto_odonto_perc_var.get() or 0)
